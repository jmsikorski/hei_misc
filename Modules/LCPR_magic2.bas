...
add_job:
    ...
    With rep_wb.Worksheets("REPORT")
        ...
        Set r_rng = .Range("rep_start").Offset(0, 7)
        For i = 0 To 2
            r_rng.Offset(0, i).Copy
            .Range(r_rng.Offset(0, i), r_rng.Offset(0, i).End(xlDown)).PasteSpecial xlPasteFormulas
        Next i
        Set r_rng = .Range("rep_start").Offset(0, 13)
        For i = 0 To 2
            r_rng.Offset(0, i).Copy
            .Range(r_rng.Offset(0, i), r_rng.Offset(0, i).End(xlDown)).PasteSpecial xlPasteFormulas
        Next i
